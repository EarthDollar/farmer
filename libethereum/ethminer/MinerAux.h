			...
			try
			{
				...
				while (!completed)
				{
					...
					if (m_submitHashrate)
					{
						...
						try
						{
							rpc.ed_submitHashrate(toJS((u256)rate), "0x" + id.hex());
						}
				...
				{
					bool ok = rpc.ed_submitWork("0x" + toString(solution.nonce), "0x" + toString(current.headerHash), "0x" + toString(solution.mixHash));
					if (ok)
